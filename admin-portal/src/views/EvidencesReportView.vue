  ...
  <div class="fade-in">
    ...
    <div class="card" style="margin-bottom: 20px;">
      ...
      <div style="font-family: var(--font-mono); font-size: 0.85rem; background: var(--bg-subtle); padding: 14px; border-radius: var(--radius); border: 1px solid var(--border-subtle); color: var(--text-body); line-height: 1.6;">
        ...
        <div style="margin-top: 6px;">📊 Resumen_Evidencias_Actividades.xlsx (Especificando actividades, fecha, supervisores y conteo de fotos)</div>
      </div>
    </div>
      ...
      <table v-else>
        ...
        <tbody>
          <tr v-for="act in previewActivities" :key="act.id">
            ...
            <td>{{ getUserDisplayName(act.user) }}</td>
            <td>
...
async function fetchPreviewActivities() {
  ...
  try {
    const params = {}
    if (filterFecInicio.value) params.fecha = filterFecInicio.value
